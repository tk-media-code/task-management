...
>
> **AI（Claude Code）向けの正本は [`.claude/rules/development-flow.md`](./.claude/rules/development-flow.md)** です。
...
### Claude Code では機械的に強制される

`.claude/settings.json`（PreToolUse）が、`git push`を含むコマンドの実行を検知し、上記のチェックを自動実行します。失敗すると**コマンドの実行自体がブロックされます。** 判定ロジックは`.claude/hooks/lib/guard-core.cjs`が持っており、`.claude/hooks/guard.cjs`はその入出力を変換するだけです。
